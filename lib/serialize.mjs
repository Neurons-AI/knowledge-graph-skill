...
export async function toKGMLAsync(opts = {}) {
  ...
  // Determine rendering strategy based on size
  let maxChildDepth = 3;    // how deep to render tree
  let maxAttrLen = 40;      // max attr value display length
  let showAllRelations = true;

  if (totalEntities > 400) {
    maxChildDepth = 1;
    maxAttrLen = 25;
    showAllRelations = false; // only show relation summary
